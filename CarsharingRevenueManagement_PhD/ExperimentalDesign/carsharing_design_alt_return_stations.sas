...
%mktruns(4 5)
%mktex(4 5, /* factor level list for one alternative */
n=20, options=noqc) /* number of candidate alternatives */
data final;
   if _n_ = 1 then do;  
      f1 = 1; f2 = 0; f3 = 0; x1 = . ; x2 = .; output;
      end;
   set design;
   f1 = .;
   f2 = (x1 <= 4) * (x2 < 5);
...
value cf 1 = 0 2 = 20 3 = 40 4 = 60 5 = 80;
run;
data ChoiceDesign3;
set best;
format x1 sf. x2 cf. ;
...
%mktblock(data=ChoiceDesign3, nalts=3, nblocks=5, factors=x1-x2, seed=472)
proc print label; var x1-x2; id set; by set; run;
%mktdups(generic, data=ChoiceDesign3, factors=x1-x2, nalts=3)
proc print; run;
